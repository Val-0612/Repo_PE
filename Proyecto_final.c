...
typedef struct {
    ...
    int primerConsumoRealizado;// 1= primer consumo en el turno realizado, 0= primer consumo del turno
} Ficha;
    ...
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 8; j++) {
            if((i + j) % 2 == 1) {
                ...
                fichas[0][idx].primerConsumoRealizado = 0;
                idx++;
            }
        }
    }
...
// ============== MOVER FICHA ==============
int moverFicha(int jugadorActual, int resultado[2], int dest[2]){
    int filaOrigen, colOrigen, filaDestino, colDestino, encontrada, repetirSolicitud, moverse;
    ...
    do{
        ...
        if(encontrada==1 && resultado[0]==jugadorActual){
            ...
            if(distanciaFila == 2 && distanciaCol == 2){
                ...
                int dirFila = (filaDestino - filaOrigen) / 2;
...
int consumirFicha(int filaOrigen, int colOrigen, int filaDestino, int colDestino, int jugadorActual){
    ...
    if(buscarFichaEnPosicion(filaMedio, colMedio, resultado)){
        ...
        if(resultado[0] != jugadorActual){
            ...
            if(fichas[jugadorActual][resultado[1]].primerConsumoRealizado == 0){
